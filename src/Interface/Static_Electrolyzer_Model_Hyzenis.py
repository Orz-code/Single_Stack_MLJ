...
class AWE_Electrolyzer_Static():
    # 初始化函数
    def __init__(self, Diameter_Electrode, Width_Cell, Num_Cells, Lye_flow_min, Lye_flow_max):
        """电解槽初始化函数
        ...
        # 热模型
        self.Heat_capacity_lye = 3.2
        self.Heat_capacity_stack = 880000
        self.Surface_heat_transfer_coefficient = 30.48
        self.tg1 = 1
        self.tc1 = 1.01
        self.tc2 = 0.95
    ...
    def Vrev_Vtn_cal(self, Lye_temp, Temp_out, Pressure):
        """
        可逆电压的计算公式（支持 float / np.ndarray / pd.Series）
    
        Args:
            Lye_temp (float / np.ndarray / pd.Series): 进口温度 ℃
            Temp_out (float / np.ndarray / pd.Series): 出口温度 ℃
            Pressure (float / np.ndarray / pd.Series): 压力 MPa
    
        Returns:
            Vrev_cell, Vtn_cell (float / np.ndarray / pd.Series): 可逆电压、热中性电压
        """
    
        # 转 numpy 数组
        Lye_temp = np.asarray(Lye_temp)
        ...
        # 熵计算
        S_H2 = Cp0_H2 * np.log(Temp_work / Tref) - R_constant * np.log(Pressure / Pref) + S0_H2
        S_O2 = Cp0_O2 * np.log(Temp_work / Tref) - R_constant * np.log(Pressure / Pref) + S0_O2
        S_H2O = Cp0_H2O * np.log(Temp_work / Tref) + S0_H2O
    
        Delta_S = S_H2 + 0.5 * S_O2 - S_H2O
        Delta_G = Delta_H - Temp_work * Delta_S
    
        Vrev_cell = Delta_G / (z * Faraday_constant)
    ...
    def Electrolytic_voltage_cal(self, Lye_temp, Temp_out, Current_density, Lye_flow, Pressure):
        ...
        Args:
            Lye_temp (float / np.ndarray / pd.Series): 进口温度，即碱液温度 ℃
            Temp_out (float / np.ndarray / pd.Series): 出口温度℃
            Current_density (float / np.ndarray / pd.Series): 电流密度 A/m2
            Lye_flow (float / np.ndarray / pd.Series): 碱液流量 m3/h
            Pressure (float / np.ndarray / pd.Series): 压力 MPa

        Returns:
            float / np.ndarray / pd.Series: 电解小室电压 V
        """

        # 反应可逆电压（假设该函数本身也支持向量）
        Vrev_cell = self.Vrev_Vtn_cal(
            Lye_temp=Lye_temp,
            Temp_out=Temp_out,
            Pressure=Pressure
        )[0]

        # 转为 numpy array（Series 会保留 index）
        Lye_temp = np.asarray(Lye_temp)
        Temp_out = np.asarray(Temp_out)
        Current_density = np.asarray(Current_density)
        Lye_flow = np.asarray(Lye_flow)

        # 避免除零或无效值
        Lye_temp = np.maximum(Lye_temp, 1e-10)
        Temp_out = np.maximum(Temp_out, 1e-10)
        ...
        # 工作温度
        temp_work = (
            (1 - lambda_flow) * Lye_temp * self.c1
            + lambda_flow * Temp_out * self.c2
        )

        # log 输入项
        log_input = (
            (self.t1 + self.t2 / temp_work + self.t3 / temp_work**2)
            * Current_density + 1
        )
        log_input = np.maximum(log_input, 1e-10)

        # 电解电压
        Vcell = (
            (self.r1 + self.r2 * temp_work) * Current_density
            + self.s1 * np.log(log_input)
            + Vrev_cell
        )
