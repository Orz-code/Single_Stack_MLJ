...
class Cols:
    ...
    cell_voltage_calculated = 'cell_voltage_calculated' # V 根据经验公式计算的电池电压
    pressure_H_cat = 'pressure_H_cat' # Mpa 氢侧压力
    pressure_O_ano = 'pressure_O_ano' # Mpa 氧侧压力
    ...
    temp_aver = 'temp_aver' # ℃，槽体平均温度
    temp_environment = 'temp_environment' # ℃，环境温度
    H_production_rate = 'H_production_rate' # m3/h，产氢速率
    O_production_rate = 'O_production_rate' # m3/h，产氧速率
    H_production_accumulated = 'H_production_accumulated' # m3，累计产氢量
    O_production_accumulated = 'O_production_accumulated' # m3，累计产氧量
    HTO = 'HTO' # %, 氧中氢
    OTH = 'OTH' # %，氢中氧
    delta_temp_out = 'delta_temp_out' # ℃/s 出口温度变化率
    cooling_valve = 'cooling_valve' # % 冷却阀门开度
    heating = 'heating' # % 加热器开度

    # 控制变量
    lye_flow_target = 'lye_flow_target'
    lye_temp_target = 'lye_temp_target'
